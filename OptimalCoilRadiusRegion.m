...
S = [1/E1 -(u_12/E1) -(u_12/E1) 0 0 0;
  ...
  for i = 1:length(R)    
    for j = 1:length(Pi)
        ...
        x1 = (r0+u_r(end))-((2*(r0+u_r(end)))^2/(16*R(i)));
        x2 = -(r0+u_r(end))-((2*(r0+u_r(end)))^2/(16*R(i)));
%         x = x2:0.000005:x1;

        tau_torsion(j) = (F*R(i).^2*x1)./(J(j)*(R(i)-((2*(r0+u_r(end)))^2/(16*R(i)))-x1));
        tau_direct(j) = F/A(j);
...
% %     %2 Newtons
%     Delta(i,j) = (2*R(i).^2*23*Gamma_z_theta_oi(i,j)*pi)/(r0+u_r(end));
...
s = surf(R,Pi,Strain')
contour3(R,Pi,Strain',[30 25 20 15 10 5 0],'-','Linewidth',1.7,'Color',[0.1 0.7 0.7])
...
% 
% 
 xlabel('Coil Radius, R (mm)'); %xlim([0 90])
...
% plot(a,Strain(:,101),'-','Linewidth',1.7,'Color',[0.1 0.7 0.7])
% plot(a,Strain(:,101),':','Linewidth',1.7,'Color',[0.1 0.8 0.5])
plot(a,Strain(:,101),'-.','Linewidth',1.7,'Color',[0.1 0.2 0.7])
%  xlabel('Fiber Angle, \alpha ( ^\circ )'); xlim([0 90])
 ylabel('Act. Strain, {\it ɛ} (%)'); ylim([-50 100])
% 
grid on 
set(gca,'GridColor',[0.1 0.2 0.9]) % a bluish color
